...
function readResponseYears() {
  if (xmlHttp.readyState == 4 && xmlHttp.status == 200) {
		...
		document.myform.year_begin.options.length = 0;
		document.myform.year_end.options.length = 0;
				
		if (long == 2)
		{
		var myBegin =xmlDoc.getElementsByTagName('yearbegin')[0].firstChild.data;
		var myEnd =xmlDoc.getElementsByTagName('yearend')[0].firstChild.data;
		
		if (myEnd == "none") { myEnd = ""; }
		
		document.myform.year_begin.options[0] = new Option(myBegin);
		document.myform.year_end.options[0] = new Option(myEnd);
		
		}	else {
		 document.myform.year_begin.options[0] = new Option('Выберите');
  		 	for (i=1; i<long+1; i++)	{
			var num = i -1;
			var myBegin =xmlDoc.getElementsByTagName('yearbegin')[num].firstChild.data;
			 document.myform.year_begin.options[i] = new Option(myBegin);
		}
	}
}	}
////////////////////////////////////////////////////////////////////////////////////
...
if (xmlHttp) 	  { 	 
	  ...
	  yearbegin = encodeURIComponent(yearbegin);
	  		...
	  		if (xmlHttp.readyState == 4 || xmlHttp.readyState == 0) 
		   {
			...
			xmlHttp.send("model=" + model + "&year_begin=" + yearbegin);
			}
}	}
