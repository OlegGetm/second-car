...
function editedArticle(idArticle, numParag)	{
			if (xmlHttp) 	  { 	 
	  		if (xmlHttp.readyState == 4 || xmlHttp.readyState == 0) 	 {
			
			var url = '../_common/ajax/replicator/edit_article.php';

var textarea = document.getElementById(numParag);
var tt = textarea.value;
var params = 'tt=' + encodeURIComponent(tt) + '&article_id=' + idArticle + '&num_parag=' + numParag;

			xmlHttp.open("POST", url, true);
			xmlHttp.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded;');
			xmlHttp.onreadystatechange = editArticle;
			xmlHttp.send(params);
			}
}   }


function editArticle(numParag) {
  if (xmlHttp.readyState == 4 && xmlHttp.status == 200) {
  
  //responseXml = xmlHttp.responseXML;
  //xmlDoc = responseXml.documentElement;
  //var aText =xmlDoc.getElementsByTagName('text')[0].firstChild.data;
  
//var textarea = document.getElementById(numParag);
//textarea.setAttribute("style", "background-color:#ccc");
alert('Текст  статьи изменен');
} }
...
eButton.setAttribute("style", "margin:0px 20px 4px 8px");
...
var  tt = (document.getElementById('ta_' +idDiv)).value;		//  текст из  textarea
			...
			var url = '../_common/ajax/replicator/edit_text_replica.php';

var params = 'tt=' + encodeURIComponent(tt) + '&id_replica=' + idDiv;
...
var url = '../_common/ajax/replicator/edit_status_replica.php';

var params = 'id_art_dial=' + idReplic + '&interest=' + radioValue;
...
function confirmEditStatus() {
		...
		responseXml = xmlHttp.responseXML;
  		xmlDoc = responseXml.documentElement;
 	 	var idRepl =xmlDoc.getElementsByTagName('id_art_dial')[0].firstChild.data;
...
function deleteRaw(idReplic)	{
	...
	if (confirm(confirmText)) 	{
			...
			var url = '../_common/ajax/replicator/delit_replica.php';
			var params = 'id_art_dial=' + idReplic;
