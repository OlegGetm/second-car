// JavaScript Documentvar userLoggedIn = userLoggedIn || false;
...
window.addEvent('domready', function() {
	var btn = $('showInsurCalc');
	if (!btn)
		return;
	var wrapper = $('copyrights').getElement('.counters');
	var pixel = new Element('img', {src: 'http://ads.adfox.ru/59610/getCode?p1=buhm&p2=v&ptrc=b&pfc=hcmb&pfb=roxt&pr=edduxug', width: 1, height: 1});
	pixel.inject(wrapper);
	var counter;
	btn.addEvent('click', function() {
		var pr = Math.floor(Math.random() * 1000000);
		if (counter)
			counter.dispose();
		counter = new Element('img', {src: 'http://ads.adfox.ru/59610/goLink?p1=buhm&p2=v&p5=roxt&pr=' + pr, width: 1, height: 1});
		counter.inject(wrapper);
	});
});

window.addEvent('domready', function() {
	var wrapper = $(document.body).getElement('.filter-cities');
	if (!wrapper)
		return;
	var buttons = wrapper.getElements('span[id]');
	var current = Cookie.read('drive_active_city') || 'all';
	if (document.location.hash.length > 1)
		current = document.location.hash.substring(1).split('-')[0];
	var dealers = $$('.dealer-minicard');
	
	var filter = function(city) {
		buttons.each(function(element) {
			if (element.id.substring(7) == city)
				element.addClass('active');
			else
				element.removeClass('active');
		});
		dealers.each(function(element) {
			if (city == 'all' || element.hasClass('city-' + city))
				element.removeClass('dealer-hidden');
			else
				element.addClass('dealer-hidden');
		});
		var c = document.location.hash.substring(1).split('-');
		document.location.hash = c.length >= 2 ? '#' + city + '-' + c[1] : '#' + city;
	}
	
	wrapper.addEvent('click:relay(span)', function(event, target) {
		var city = target.id.substring(7);
		buttons.removeClass('active');
		target.addClass('active');
		filter(city);
		if ($('YMapsList'))
			filterDealers();
		Cookie.write('drive_active_city', city, {domain: 'drive.ru', path: '/', duration: 365});
	});
	
	filter(current);
	if (!wrapper.getElement('.active')) {
		current = 'all';
		filter('all');
	}
});
...
window.addEvent('domready', function() {
	var wrapper = $(document.body).getElement('.filter-brands');
	if (!wrapper)
		return;
	var buttons = wrapper.getElements('span[id]');
	var current = Cookie.read('drive_active_brand') || 'all';
	if (document.location.hash.length > 1) {
		var c = document.location.hash.substring(1).split('-');
		if (c.length >= 2)
			current = c[1];
	...
	var filter = function(brand) {
		buttons.each(function(element) {
			if (element.id.substring(13) == brand)
				element.addClass('active');
			else
				element.removeClass('active');
		});
		dealers.each(function(element) {
			if (brand == 'all' || element.hasClass('brand-' + brand))
				element.removeClass('dealer-brand-hidden');
			else
				element.addClass('dealer-brand-hidden');
		});
		var c = document.location.hash.substring(1).split('-');
		document.location.hash = '#' + c[0] + '-' + brand;
	}
	
	wrapper.addEvent('click:relay(span)', function(event, target) {
		var brand = target.id.substring(13);
		buttons.removeClass('active');
		target.addClass('active');
		filter(brand);
		if ($('YMapsList'))
			filterDealers();
		Cookie.write('drive_active_brand', brand, {domain: 'drive.ru', path: '/', duration: 365});
	});
